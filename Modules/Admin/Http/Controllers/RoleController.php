...
class RoleController extends Controller
{
    ...
    public function index() {
        $this->checkPermissionRedirect(Actions::VIEW_ROLES, 'admin');

        $data['addPermission'] = $this->checkHasPermission(Actions::CREATE_ROLES, 'admin');
        $data['editPermission'] =$this->checkHasPermission(Actions::EDIT_ROLES, 'admin');
        ...
        $data['scripts'] = [];
        $data['addRoute'] = 'admin.roles.create';

        $data['viewSingle'] = false;

        $data['getData'] = ['url' => 'admin.roles.get-data', 'holder' => 'table-holder'];
    ...
    public function create() {
        $this->checkPermissionRedirect(Actions::CREATE_ROLES, 'admin');

        $roles = ['View Permissions', 'Create Permissions', 'Edit Permissions'];
        if (!auth()->user()->hasRole('Super Admin')) {
            $permissions = Permission::where('guard_name', '=', 'admin')->whereNotIn('name', $roles)->get();
        }
        else {
            $permissions = Permission::whereNotIn('name', $roles)->orderBy('guard_name', 'asc')->get();
        }

        $view = View::make('admin::roles.create', ['permissions' => $permissions])->render();
    ...
    public function edit(Request $request, $id) {
        $this->checkPermissionRedirect(Actions::EDIT_ROLES, 'admin');

        $data['role'] = $this->roleService->getRole($id);
        
        $roles = ['View Permissions', 'Create Permissions', 'Edit Permissions'];
        if (!auth()->user()->hasRole('Super Admin')) {
            $data['permissions'] = Permission::where('guard_name', '=', 'admin')->whereNotIn('name', $roles)->get();
        }
        else {
            $data['permissions'] = Permission::whereNotIn('name', $roles)->orderBy('guard_name', 'asc')->get();
        }
