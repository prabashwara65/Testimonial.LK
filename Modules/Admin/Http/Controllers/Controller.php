    ...
    protected function checkPermissionRedirect($action, $guard)
    {
        try {
            if (!Auth::user()->hasPermissionTo($action, $guard)) {
                $redirect = '';
                if(! Request::ajax()){
                    Redirect::route('admin.permission-denied')->send();
                }

                $outPutArray = array('status' => 'error', 'message' => 'You do not have permission to perform this action: "'.$action.'"', 'data' => route('admin.permission-denied'), 'redirect' => $redirect, 'notifyType' => 'message');
