...
class ActionLogService extends MainService
{
    ...
    {
        $actionLog = ActionLog::where('vendor_company_id', 0)
            ->whereRaw("DATE(created_at) BETWEEN '".$filterForm['start_date']."' AND '".$filterForm['end_date']."' ");

        if (!empty($search)) {
            $actionLog = $this->generateWhereLikeQuery($actionLog, $columns, $search);
        }
        if (!empty($orderBy)) {
        ...
        if ($filterForm['user'] != 'Any') {
            $actionLog->where('user', $filterForm['user']);
        }
        ...
        foreach ($rows as $log) {
            $temp = [
                ...
                $this->generateActionButtons('admin.action-log', $log->id, ['view' => Actions::EDIT_USERS])
            ];
