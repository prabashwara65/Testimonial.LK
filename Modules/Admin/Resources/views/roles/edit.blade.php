{{ Form::open(array('url' => route('admin.roles.update', $role['id']), 'class'=>'text-form', 'method' => 'put', 'onsubmit' => 'return false;')) }}
<div class="modal-header">
    <h4 class="modal-title c-grey-900 ">Edit '{{$role['name']}}'</h4>
            ...
            <div class="col-sm-9">
                <select name="permissions[]" id="permissions" class="form-control" multiple data-live-search="true">
                    @foreach ($permissions as $permission)
                        <option value="{{$permission->id}}" @if(in_array($permission->id, $role['permissions'])) selected @endif>{{$permission->name}} - ({{$permission->guard_name}})</option>
                    @endforeach
                </select>
