{{ Form::open(array('url' => route('admin.roles.store'), 'class'=>'text-form', 'onsubmit' => 'return false;')) }}
            ...
            <div class="col-sm-9">
                <select name="permissions[]" id="permissions" class="form-control" multiple data-live-search="true">
                    @foreach ($permissions as $permission)
                        <option value="{{$permission->id}}" >{{ucfirst($permission->name)}} - ({{$permission->guard_name}})</option>
                    @endforeach
                </select>
