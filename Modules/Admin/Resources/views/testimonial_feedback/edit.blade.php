{{ Form::open(array('url' => route('vendor.testimonial-feedback.update', $testimonial['id']), 'class'=>'text-form', 'method' => 'put', 'onsubmit' => 'return false;')) }}
...
<div class="modal-body">
    ...
    <div id="reject-panel" class='col-lg-10 offset-lg-1' @if($testimonial['status'] != 'reject') style="display: none;" @endif>
        <div class="form-group row">
            ...
            <div class="col-sm-8">
                <div class="rate">
                    @for($i = config('settings.rating-score'); $i > 0 ; $i--)
                        <input type="radio" id="star{{$i}}" name="rating" value="{{$i}}" @if($testimonial['rating'] == $i) checked @endif/>
                        <label class="star" for="star{{$i}}" title="{{ config('settings.star-'.$i) }}" data-toggle="tooltip" data-placement="top">{{$i}} star</label>
                    @endfor
                </div>
            </div>
        </div>
