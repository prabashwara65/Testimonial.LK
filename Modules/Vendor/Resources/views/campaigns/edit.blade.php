{{ Form::open(array('url' => route('vendor.campaigns.update', $campaign['id']), 'class'=>'text-form', 'method' => 'put', 'onsubmit' => 'return false;')) }}
<div class="modal-header">
    <h4 class="modal-title c-grey-900 ">Edit Campaign</h4>
...
<div class="modal-body">
    <div class="col-lg-12 offset-lg-0">
        <div class="form-group row">
            ...
            <div class="col-sm-4">
                <select name="campaign_type" id="campaign_type" class="form-control select2" style="width: 100%" data-live-search="true">
                    <option value="Single" @if($campaign['campaign_type'] == 'Single') selected @endif>Single</option>
                    <option value="Multiple" @if($campaign['campaign_type'] == 'Multiple') selected @endif>Multiple</option>
                </select>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-4">
                <select name="target_id" id="target_id" class="form-control select2" style="width: 100%" data-live-search="true">
                    @foreach ($targets as $target)
                        <option value="{{$target['id']}}" @if($campaign['target_id'] == $target['id']) selected @endif>{{ucfirst($target['target_name'])}}</option>
                    @endforeach
                </select>
            ...
            <div class="col-sm-4">
                <select name="response_type" id="response_type" class="form-control select2" style="width: 100%" data-live-search="true">
                    <option value="Questionnaire" @if($campaign['response_type'] == 'Questionnaire') selected @endif>Questionnaire</option>
                    <option value="Record" @if($campaign['response_type'] == 'Record') selected @endif>Record</option>
                    <option value="Both" @if($campaign['response_type'] == 'Both') selected @endif>Both</option>
                </select>
                ...
                <select name="branches[]" id="branches" class="form-control select2 load-data-on-change" data-url="{{$loadEmployeeUrl}}" data-target="#employees" style="width: 100%" multiple data-live-search="true">
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{$branch['id']}}"
                            @if(in_array($branch['id'], $campaign['branches']))
                                selected
                                <?php
                                    array_push($allemployees, \App\Models\Vendor::where('branch_id', '=', $branch['id'])->where('status', 1)->get());
                                ?>
                        ...
                        @foreach ($allemployees as $employees)
                            @foreach ($employees as $employee)
                                <option value="{{$employee['id']}}"
                                    @if(in_array($employee['id'], $campaign['employees']))  selected @endif >
                                    {{ $employee->emp_id . ' - ' . $employee->name . ' ' . $employee->last_name }}
                                </option>
                            @endforeach
                        @endforeach
                    @endif
        ...
        <div class="form-group row">
            <label for="incentive_rate" class="col-sm-2 col-form-label">Incentive Rate<i class="required-star"></i></label>
            <div class="col-sm-4">
                <input type="text" name="incentive_rate" id="incentive_rate" class="form-control" value="{{$campaign['incentive_rate']}}">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-4">
                <input type="text" name="start_date" id="start_date" class="form-control datepicker" data-date-format="yyyy-mm-dd" value="{{$campaign['start_date']}}">
            ...
            <div class="col-sm-4">
                <input type="text" name="end_date" id="end_date" class="form-control datepicker" data-date-format="yyyy-mm-dd" value="{{$campaign['end_date']}}">
        ...
        <div class="campaign-holder">
            @foreach($campaign['products'] as $productId)
                @component('vendor::campaigns.campaign_template', ['loadSubproductUrl' => $loadSubproductUrl, 'count' => $productId."E", 'productId' => $productId, 'products' => $products, 'subproducts' => $campaign['subproducts']]) @endcomponent
            @endforeach
        </div>
